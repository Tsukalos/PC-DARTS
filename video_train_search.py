...
parser = argparse.ArgumentParser("video_train_search")
...
parser.add_argument('--train_portion', type=float, default=0.65, help='portion of training data')
parser.add_argument('--unrolled', action='store_true', default=False, help='use one-step unrolled validation loss')
parser.add_argument('--arch_learning_rate', type=float, default=6e-4, help='learning rate for arch encoding')
parser.add_argument('--arch_weight_decay', type=float, default=1e-3, help='weight decay for arch encoding')
parser.add_argument('--fbs_channels', type=int, default=32, help='Frequency band selection. The number of dct channels used.')
...
args.save = '{}/search-{}-{}'.format(root_save_dir,
...
def main():
    ...
    model = Network(args.init_channels, VIDEO_CLASSES, args.layers,
                    criterion, dct_channels=dct_coefficients)
    model = model.cuda()
    ...
    train_transform, __ = utils._data_transforms_ucf101_train(args)
    train_data = UCF101DCTDataset_MV("../data/UCF101jpg_/UCF-101",
                                    ...
                                    num_coeff=dct_coefficients, seed=args.seed, shuffle=True, train=True)
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, epoch):
    ...
    for step, (input_list, _, target) in enumerate(train_queue):
        model.train()
        ...
        try:
            input_search, _, target_search = next(valid_queue_iter)
        except:
            valid_queue_iter = iter(valid_queue)
            input_search, _, target_search = next(valid_queue_iter)
        input_search = Variable(
            input_search.transpose(1, 0), requires_grad=False).cuda()
        target_search = Variable(
            target_search, requires_grad=False).cuda(non_blocking=True)

        if epoch >= 15:
            architect.step(input, target, input_search,
                           target_search, lr, optimizer, unrolled=args.unrolled)
...
def infer(valid_queue, model, criterion):
    ...
    model.eval()
    with torch.no_grad():
        for step, (input_list, _, target) in enumerate(valid_queue):
            # input = input.cuda()
